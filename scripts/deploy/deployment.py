...
class VLLMDeployment(
    LRUMixin, RoutingMixin, GossipMixin, ParsingMixin, InferenceMixin,
):
    def __init__(self):
        ...
        logger.info(
            f"[vllm] LoRA adapters ({len(self.lora_names)}): {self.lora_names}"
        )
    ...
    async def chat_completions(self, request: Request):
        ...
        try:
            ...
            logger.error(
                f"[endpoint] /v1/chat/completions parse error: {e}\n"
        ...
        if ROUTING_MODE == "memory":
            target_ip = self._choose_target_node_memory(
                adapter_name, parsed["client_ip"]
            )
        else:
            target_ip = self._choose_target_node_baseline(
                adapter_name, parsed["client_ip"]
            )
        ...
        if is_local:
            ...
            response = await self._forward_chat_request(
                target_ip, body, request_id
            )

        e2e_time_ms = (time.perf_counter() - e2e_start) * 1000
        logger.info(
            f"[e2e] request_id={request_id} status={response.status_code} "
            f"total_ms={e2e_time_ms:.1f} forwarded={not is_local} "
            f"served_by={target_ip}"
        )
        self.metrics.log(
            "e2e_latency",
            request_id=request_id,
    ...
    async def health(self):
        ...
        return JSONResponse({
            "status": "ok", "node": self.my_ip, "ongoing": self._ongoing,
        })
    ...
    async def debug_state(self):
        return JSONResponse({
            ...
            "gossip_task_active": (
                self._gossip_task is not None and not self._gossip_task.done()
            ),
        })
    ...
    async def cluster_state(self):
        ...
        return JSONResponse({
            "ts": time.time(), "source_node": self.my_ip, "nodes": nodes,
        })
